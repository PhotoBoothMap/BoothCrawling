...
class PhotoBoothCrawler:
    def __init__(self, input_search_list, brand_info):
    ...
    def set_data_to_list(self, log_data_list) -> List[BoothData]:
        booth_list = list()
        for log_data in log_data_list:
            place_data_list = log_data["place"]
            for place in place_data_list:
                booth_data = BoothData(
                    confirm_id=place.get("confirmid", None),
                ...
                filtered_booth_data = self.filter_booth_data(booth_data)

                if filtered_booth_data is not None:
                    booth_data = self.convert_brand_name_to_id(filtered_booth_data)
                    booth_list.append(booth_data)

        return booth_list

    @staticmethod
    def filter_booth_data(booth_info: BoothData) -> Optional[BoothData]:
    ...
    def convert_brand_name_to_id(self, booth_data: BoothData) -> BoothData:
